...
(let ((warnings 0))
  (handler-bind ((warning (lambda (condition)
                            (incf warnings)
                            (format *error-output* "~&lane-check warning: ~a~%" condition))))
    (asdf:load-system "my-system" :force '("my-system")))
...
```bash
# toolchain
sbcl --version | grep -Fx 'SBCL 2.6.8'
# check
XDG_CACHE_HOME="$(mktemp -d)" sbcl --noinform --non-interactive --disable-debugger --no-sysinit --no-userinit --load scripts/check.lisp
# test
XDG_CACHE_HOME="$(mktemp -d)" sbcl --noinform --non-interactive --disable-debugger --no-sysinit --no-userinit --load scripts/test.lisp
```

The fresh `XDG_CACHE_HOME` keeps each run from reusing cached FASLs: ASDF output translations follow it, and an edit landing in the same second as the previous compile can otherwise load the stale FASL. `:ignore-inherited-configuration` keeps a developer's source registry out of the run (`--no-sysinit --no-userinit` alone doesn't). When Qlot owns dependencies, run the same scripts through `qlot exec`. `scripts/lane-conformance.py common-lisp` runs these gates on a fixture system at every engineering-core release.
