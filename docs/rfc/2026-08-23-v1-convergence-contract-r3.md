...
# RFC: engineering-core v1.0 convergence contract — revision 3

## Status and authority

- **RFC state:** revised proposal; not accepted and not an execution or release authorization.
- **Supersedes for review:** the historical RFC and `docs/rfc/2026-08-23-v1-convergence-contract-r2.md`; both review attempts remain immutable history.
...
- **Lifecycle inputs:** the problem/evidence notes and `docs/project/2026-08-23-v1-convergence-review-set-plan-r3.md`.
...
- the rendered-product manifest schema and candidate-independent inventory cover every stable lane, discipline, addendum, template, profile, policy/document projection, help surface, and no-effect example, with oracles for source/root/package and wheel/sdist parity, links/front matter, deterministic promises, hostile rendering, and checkout-local path absence;
...
Before the first run, each G0-B journey manifest records: starting/ending checkpoints; exact argv or candidate-runbook path/digest from the rendered-product manifest; cwd/environment/network/candidate/timeout; expected status/schema; allowed/prohibited effects; validation and recovery. Invocation, approval, or final validation alone is not PASS. The reviewed plan digest from journey 3 is the input bound by journey 4; any state/runbook/oracle drift requires renewed review.
...
6. for existing adopters, complete the selected transition/migration without claiming compatibility, then prove exactly one active v1 pin/selection, no active pre-v1 resolution or mixed-runtime dependency, and preserved owner truth; for clean-slate adoption, repeat plan/apply and prove idempotence;
7. inject malformed and unavailable pins, malformed policy, and interruption/failure at **every durable mutation boundary**. A transaction design may replace per-boundary recovery only when fault injection independently proves atomic visibility and rollback at every boundary. Each failure observes its frozen pre-effect limits, invokes the candidate public recovery path, and leaves no fabricated state, accepted partial write, helper, or process;
...
G0-A1 requires accepted decision/ADR and immutable closure. G0-B depends on every G4-A owner/content-owner task; G5 depends on every G1–G4 production/review task. Deferrals cannot waive edges. Cross-repo tasks exist only in owning repos after owner acceptance.
