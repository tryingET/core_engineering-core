...
# RFC: engineering-core v1.0 convergence contract — revision 5

## Status and authority

- **RFC state:** revised proposal; not accepted and not an execution or release authorization.
- **Supersedes for review:** the historical RFC plus R2–R4; every prior review attempt remains immutable history.
...
- **Lifecycle inputs:** the problem/evidence notes and `docs/project/2026-08-23-v1-convergence-review-set-plan-r5.md`.
...
G0-A2 freezes the compatibility-manifest schema, classification rules, and candidate-independent inventory method. G4-A content remains pilot/candidate input, not a frozen v1 surface. After G4-A closes, G0-A3 materializes and G0-B verifies/freezes the final candidate-bound manifest using these classifications:
...
1. **Stage-correct binding.** G0-A1 binds RFC/authority; G0-A2 binds candidate-independent templates; G4-A binds pilot/participant revisions; G0-A3 binds the authorized source-to-candidate transition; G0-B verifies candidate instances; G1–G5 bind one candidate/population digest.
...
After all G0-A2 implementation and G4-A production/review/disposition tasks complete, a read-only G0-A3 verifier emits an immutable **candidate-admission record**. It binds the exact base commit/tree; every reviewed implementation output/task/review digest; accepted G4 content bytes and negative-lineage refs; all frozen population, protocol, template, fixture, oracle, impact-matrix, and toolchain digests; the exact candidate task; permitted content-path/diff and effect allowlists; and target proof branch/channel. Any missing, stale, extra, or changed input blocks admission and reruns its owning stage.

Only that record releases one exact candidate-preparation task. The task runs on an isolated non-`main` branch/PR and may produce only the admitted source transition: integrate bound bytes, fill declared candidate-only materialization fields, set final metadata, and push one immutable proof commit. Every output byte maps to an admitted input or declared deterministic materialization. Candidate checks use only G0 candidate-conformance and synthetic study-disjoint fixtures; they cannot execute or expose G1–G4 corpora/outcomes. The task cannot merge, tag, create a Release, upload public assets, mutate participants, or claim PASS. Drift/failure yields no candidate; correction requires a fresh admission record and candidate identity.
...
- candidate tree/base/diff, task identity, materialized fields, and every input/output digest exactly satisfy the G0-A3 admission record; undeclared bytes or stale inputs fail G0-B;
...
- remote tag/Release are absent, while G0-B records the admission record plus exact candidate, immutable v1.0 compatibility/assertion baseline, population, protocol/template/instance/transition, effects, G4 inclusion, toolchain, and workflow digests.
